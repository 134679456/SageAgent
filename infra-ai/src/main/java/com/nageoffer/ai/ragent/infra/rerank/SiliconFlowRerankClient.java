...
public class SiliconFlowRerankClient implements RerankClient {
    ...
    private List<RetrievedChunk> doRerank(String query, List<RetrievedChunk> candidates, int topN, ModelTarget target) {
        ...
        try (Response response = httpClient.newCall(request).execute()) {
            if (!response.isSuccessful()) {
                ...
                log.warn("{} rerank request failed: status={}, body={}", provider(), response.code(), body);
                throw new ModelClientException(
                        provider() + " rerank request failed: HTTP " + response.code(),
            ...
            throw new ModelClientException(provider() + " rerank request failed: " + e.getMessage(), ModelClientErrorType.NETWORK_ERROR, null, e);
        }

        JsonArray results = requireResults(respJson);
        if (CollUtil.isEmpty(results)) {
            throw new ModelClientException(provider() + " rerank results is empty", ModelClientErrorType.INVALID_RESPONSE, null);
        }
    ...
    private JsonArray requireResults(JsonObject respJson) {
        if (respJson == null || !respJson.has("results")) {
            throw new ModelClientException(provider() + " rerank response missing results", ModelClientErrorType.INVALID_RESPONSE, null);
        }
        JsonArray results = respJson.getAsJsonArray("results");
        if (results == null) {
            throw new ModelClientException(provider() + " rerank response results is null", ModelClientErrorType.INVALID_RESPONSE, null);
        }
        return results;
    }
}
